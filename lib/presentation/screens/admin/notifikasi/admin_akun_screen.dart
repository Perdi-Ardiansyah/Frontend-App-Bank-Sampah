        ...
        IconButton(
          ...
          onPressed: () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => const AdminNotifikasiScreen(),
              ),
            );
          },
        ),
        if (hasUnread &&
            !state.isLoading) // Titik merah muncul jika ada yang belum dibaca
          Positioned(
            right: 12,
            top: 12,
            child: Container(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(
              'Kelola kata sandi dan pantau aktivitas notifikasi Anda di sini.',
              style: AppTextStyles.bodyMd.copyWith(
                color: AppColors.onSurfaceVariant,
                height: 1.5,
              ),
...
class _AdminNotifikasiScreenState extends ConsumerState<AdminNotifikasiScreen> {
  ...
  // 👇 1. UPDATE FILTER: Menambahkan 'Penukaran' ke dalam daftar filter
  ...
  // 👇 2. UPDATE MESIN DESAIN: Menambahkan style khusus untuk tipe Penukaran Sembako
  Map<String, dynamic> _getStyleByTipe(String tipe) {
    ...
    // Tipe 2: Penukaran Sembako / Produk (Ungu / Indigo Container)
    if (t.contains('penukaran') || t.contains('produk')) {
      return {
        'tagColor': Colors.indigo.shade50,
        'tagTextColor': Colors.indigo,
    ...
    // Tipe 3: Pencairan Dana Cash/Transfer (Oranye/Kuning)
    if (t.contains('pencairan') ||
        t.contains('cash') ||
        t.contains('transfer')) {
      return {
    ...
    // Tipe 5: Setoran Baru (Hijau)
    if (t.contains('setoran')) {
      return {
        'tagColor': AppColors.success.withOpacity(0.15),
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          TextButton.icon(
            onPressed: () =>
                ref.read(notifikasiProvider.notifier).tandaiSemuaDibaca(),
            icon: const Icon(
